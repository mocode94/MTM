    ...
    def __init__(self,code, holder, toolLenght):
        ...
        def make_entry(toolLenght):
            ...
            labels = [
                ...
                "LBREAK", "RBREAK", "DIRECT", "Max_Durchmesser", "Max_Laenge", "P2", "BC", "IKZ", "ML", "MLR", "AM", "KINEMATIC"
            ]
            ...
            for idx, label_text in enumerate(labels):
                ...
                if label_text == "CODE":
                ...
                elif label_text == "Max_Durchmesser":
                    label = Label(TNC_win, text=label_text, font=label_font)
                    label.grid(row=idx - 7, column=2, padx=10, pady=5, sticky='e')

                    # Create a combobox for the Max Durchmesser field
                    ...
                    entries["Max_Durchmesser"] = combobox

                elif label_text == "Max_Laenge":
                    label = Label(TNC_win, text=label_text, font=label_font)
                    label.grid(row=idx - 7, column=2, padx=10, pady=5, sticky='e')
                    entry = Entry(TNC_win, validate='key', validatecommand=(validate_cmd_all, '%P'), font=entry_font)
                    entry.grid(row=idx - 7, column=3, padx=10, pady=5)
                    maxLenght=float(toolLenght)
                    maxLenght=round(maxLenght,1)  # Round to 1 digit after the decimal point
                    Max_LaengeValue=maxLenght+10
                    Max_LaengeValue=str(Max_LaengeValue)
                    entry.insert(0, Max_LaengeValue)  # Set "0.5" as the default value
                    entries["Max_Laenge"] = entry
            ...
            #         # Ensure the new row has no empty values
            #         new_row = [data.get(col, '0') if data.get(col) in (None, '', ' ') else data.get(col) for col in header]

            #         # Check if the row exists in the database
            #         cursor.execute("SELECT * FROM tnc640Data WHERE CODE = ?", (data.get('CODE', ''),))
            ...
            #             cursor.execute(f"""
            #                 UPDATE tnc640Data
            #                 SET {', '.join(f'{col} = ?' for col in header[1:])}
            #                 WHERE CODE = ?
            #             """, (*new_row[1:], new_row[0]))
            #             print(f"Row with CODE {new_row[0]} updated.")
            #         else:
            #             # Insert the new row
            #             cursor.execute(f"""
            #                 INSERT INTO tnc640Data ({', '.join(header)})
            ...
            def save_all():
                ...
                # You could also handle empty string or None specifically if needed
                new_row = [item if item not in (None, '', ' ') else '0' for item in new_row]
