...
namespace DNC_CSharp_Demo.UserControls
{
    ...
    public partial class RemoteError : UserControl
    {
        ...
        private void RemoteError_Load(object sender, EventArgs e)
        {
            ...
            Console.WriteLine("RemoteError_Load");
      ...
      */
      ...
      GC.WaitForPendingFinalizers();
            ...
            
        {
            ...
            string ErrorOrInfo = DataAccess.ErrorOrInfo;
            string toolNR = DataAccess.toolNR;
            string toolN = DataAccess.toolN;
            int ErrorNR = DataAccess.ErrorNR;
            string Ursache = DataAccess.Ursache;
            string Loesung = DataAccess.Loesung;
            string MSG = DataAccess.MSG;

            try
                    {
                        int errorNumber = Convert.ToInt32(ErrorNR);

                        // Only create error entry if error number doesn't exist.
                        if (this.GetErrorByNumber(errorNumber) == null)
                        {
                            HeidenhainDNCLib.DNC_ERROR_CLASS errorClass = (HeidenhainDNCLib.DNC_ERROR_CLASS)Enum.Parse(typeof(HeidenhainDNCLib.DNC_ERROR_CLASS), ErrorOrInfo);
                            ...
                            tmpEntry = this.remoteError.CreateEntry(errorNumber, errorClass, "T" +toolNR+ " " +toolN+" "+MSG);
                            tmpEntry.Cause = Ursache;
                            tmpEntry.Action = Loesung;
