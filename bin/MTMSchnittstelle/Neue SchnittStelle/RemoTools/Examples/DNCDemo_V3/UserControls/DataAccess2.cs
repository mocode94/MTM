// ------------------------------------------------------------------------------------------------
// <copyright file="DataAccess2.cs" company="DR. JOHANNES HEIDENHAIN GmbH">
...
namespace DNC_CSharp_Demo.UserControls
{
    ...
    /// <summary>
    /// This user control is used to show the IJHDataAccess2 functionalities.
    /// </summary>
    public partial class DataAccess2 : UserControl
    {
        #region "type defs"
        #endregion
        ...
        #region "fields"
        /// <summary>
        /// A reference to the main form to access to all public or internal
        /// resources of the main form.
        /// </summary>
        private MainForm parent = null;

        /// <summary>
        /// The parent data access control. (needed to access to the data selection)
        /// </summary>
        ...
        public List<string> childListValues { get; private set; } = new List<string>();
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="DataAccess2"/> class.
        /// Copy some useful properties to local fields.
        /// </summary>
        /// <param name="mainForm">Reference to the main application Form.</param>
        /// <param name="parentDataAccess">The parent data access control.</param>
        public DataAccess2(MainForm mainForm, DataAccess parentDataAccess)
        {
            ...
            this.Disposed += new EventHandler(this.DataAccess2_Disposed);
        }
        #endregion
        ...
        /// </summary>
        /// <param name="lSubscribeHandle">Number that was returned by the method IJHDataEntry::Subscribe() to identify the data without the need for a string compare.</param>
        ...
        internal delegate void OnDataHandler(int lSubscribeHandle, DateTime timeStamp, object varData);
...
#else
                this.dataAccess.OnData += new HeidenhainDNCLib._DJHDataAccessEvents_OnDataEventHandler(this.DataAccess_OnData);
#endif
                this.initOkay = true;
        ...
        //public string tableIdent =@"\TABLE\TOOL";
        //public string primaryKeyName = @"\T";
        //public string primaryKeyValue = "291";
        ...
        public void AddRecordToTable(string primaryKeyValue)
        {
            string tableIdent = @"\TABLE\TOOL";
            string primaryKeyName = @"\T";
            //string primaryKeyValue = "1721";
            ...
            {
                Console.WriteLine("Error1 NEWRECORD");
            ...
            {
                Console.WriteLine("Error2 NEWRECORD");
            ...
            finally
            {
                ...
                Console.WriteLine("finally NEWRECORD");
        ...
        /// <summary>
        /// This method returns (a pointer to) the JHDataEntry object that is selected by the specified path.
        /// If a node is selected, the JHDataEntry object has a collection property that contains the JHDataEntry objects of the next level.
        /// </summary>
        ...
        public void GetDataEntry(string dataSelection)
        {
            ...
            try
            {
                ...
                ChildListListBox.BeginUpdate();
                PropertyListListView.BeginUpdate();
                ChildListListBox.Items.Clear();
                ...
                {
                    SetDataEntryButton.Enabled = false;
                    dataEntryList = this.mainDataEntry.childList;

                    for (int i = 0; i < dataEntryList.Count; i++)
                    {
                        innerDataEntry = dataEntryList[i];
                        //Console.WriteLine(innerDataEntry.bstrFullName);
                        ChildListListBox.Items.Add(innerDataEntry.bstrFullName);
                        childListValues.Add(innerDataEntry.bstrFullName);
        ...
        public void Subscribe(string dataSelection)
        {
            ...
            try
            {
                dataEntry = this.dataAccess.GetDataEntry(dataSelection);
                ...
                // --- subscribe --------------------------------------------------------------------------
                int subscriptionHandle = dataEntry.Subscribe(triggerCondition, triggerSampling);
        ...
        public void SwitchLogging(bool state, string path = null)
        {
            ...
            if (this.loggingActive)
            {
                ...
                this.log.Logfile = path + @"\IJHDataAccess2.log";
                this.log.Start(true);
            }
            else
            {
        ...
        /// <param name="timeStamp">Timestamp for the moment that the data was stored.  </param>
        /// <param name="varData">Object that contains the data value(s).</param>
        private void DataAccess_OnData(int subscribeHandle, DateTime timeStamp, object varData)
        {
            string varDataString = @"<null>";
            if (varData != null)
            {
                varDataString = varData.ToString();
            }

            Debug.WriteLine("IDispatch::OnData - Handle: " + Convert.ToString(subscribeHandle) + " - " + varDataString);
            this.BeginInvoke(new OnDataHandler(this.OnDataImpl), subscribeHandle, timeStamp, varDataString);
        }
        ...
        private void DataAccess2_Disposed(object sender, EventArgs e)
        {
            // --- 1. Unsubscribe all subscriptions -----------------------------------------------
            // Unsubscribe is only possible if the application is connected to the control.
                this.UnSubscribe();

            // --- 2. Release global helper objects here ------------------------------------------
            if (this.mainDataEntry != null)
            {
                Marshal.ReleaseComObject(this.mainDataEntry);
                this.mainDataEntry = null;
            }

            if (this.dataAccess != null)
            {               
                // --- 3. Unadvise all event handlers here ----------------------------------------
#if RAW_COM_EVENTS
        ...
        {
            this.SetDataEntry(VarValueTextBox.Text,out bool isError);
        }
        ...
        /// <param name="e">The parameter is not used.</param>
        private void ChildListListBox_MouseClick(object sender, MouseEventArgs e)
        {
            if (sender != null)
            {
                ...
                if (listBox.SelectedItem != null)
                {
                    ...
                    DataSelectionEventArgs args = new DataSelectionEventArgs(selectedItem);
        ...
        /// <param name="timeStamp">Timestamp for the moment that the data was stored.  </param>
        /// <param name="varData">Object that contains the data value(s).</param>
        private void OnDataImpl(int subscribeHandle, DateTime timeStamp, object varData)
        {
            Debug.WriteLine("OnDataImpl - Handle: " + Convert.ToString(subscribeHandle));
            for (int i = 0; i < this.subscriptionList.Count; i++)
            {
                if (this.subscriptionList[i].Handle == subscribeHandle)
                {
                    if (this.loggingActive)
                    {
                        this.log.LogMessage("EventTime: " + timeStamp.ToLongTimeString() + "." + timeStamp.Millisecond.ToString("000") +
                                            " ; HandleID: " + subscribeHandle.ToString() +
                                            " ; dataSelection: " + this.subscriptionList[i].DataSelection +
                                            " ; varValue: " + varData.ToString());
                    }

                    this.subscriptionList[i].Value = varData.ToString();
        ...
        public void SetDataEntry(string varValue, out bool isError)
        {
            isError = false; // Default to no error

            string SetDataEntryVar = DataAccess.SetDataEntryVar;

            this.GetDataEntry(this.parentDataAccess.DataSelectionTextBox.Text);
            ...
            catch (COMException cex)
            {
                ...
                Console.WriteLine($"ErrorCOMException wrong Entry: {SetDataEntryVar} {cex.Message}");
                //Environment.Exit(1);
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"ErrorException wrong Entry: {SetDataEntryVar} {ex.Message}");
                //Environment.Exit(1);
                isError = true;
                //string className = this.GetType().Name;
                //string methodName = MethodInfo.GetCurrentMethod().Name;
        ...
        /// </summary>
        private class SubscriptionData : IDisposable
        {
            #region "constructor & destructor"
            /// <summary>
            ...
            internal SubscriptionData(int handle, HeidenhainDNCLib.IJHDataEntry dataEntry)
            {
                this.Handle = handle;
            ...
            #region "properties"
            /// <summary>
            /// Gets reference to the JHDataEntry object for which the subscription was made for.
            /// </summary>
            internal HeidenhainDNCLib.IJHDataEntry DataEntryObject { get; private set; }
            ...
            public void Dispose()
            {
                if (this.DataEntryObject != null)
                {
                    ...
                    {
                        if (cex.ErrorCode != (int)HeidenhainDNCLib.DNC_HRESULT.DNC32_E_NOT_CONN)
                        {
                            throw cex;
                        }
                    }
