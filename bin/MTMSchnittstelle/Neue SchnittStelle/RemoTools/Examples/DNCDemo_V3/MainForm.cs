...
namespace DNC_CSharp_Demo
{
    ...
    using System.Linq; // Add this for LINQ methods like First()
    using DNC_CSharp_Demo.UserControls;
    ...
    public partial class MainForm : Form
    {
        ...
        private void Connect()
        {
            ...
            string ipAddress = null; // Declare the variable here so it's accessible outside the if block
            ...
            try
            {
                ...
                Match match = Regex.Match(firstLine, ipPattern);

                if (match.Success)
                {
                    // Extract and store the IP address in a variable
                    ipAddress = match.Groups[1].Value;
                    Console.WriteLine($"Extracted IP Address: {ipAddress}");
                }
                else
                {
                    Console.WriteLine("IP address not found in the file.");
                }
                // Use Connect() instead of ConnectRequestX() for older controls
                // like TNC 426/430 or CNC PILOT 4290 
                this.Machine.ConnectRequest(ipAddress);
                DisconnectButton.Enabled = true;
        ...
        {
            Console.WriteLine("here is Disconnect");
