...
namespace DNC_CSharp_Demo
{
    ...
    public partial class MainForm : Form
    {
        ...
        /// <summary>
        /// This is the WPF user control which shows the state machine
        /// </summary>
        private DNC_CSharp_OnStateChanged.OnStateChangedUserControl wpfOnStateChanged = null;
        #endregion
        ...
        /// <summary>
        /// Gets a value indicating whether the connected CNC control is NCK based.
        /// </summary>
        ...
        internal HeidenhainDNCLib.DNC_STATE ControlState
        {
            ...
            private set
            {
                ...
                this.wpfOnStateChanged.SetState(this.controlState);
        ...
        /// by calling IJHMachine::GetState() first.
        /// </summary>
        /// <param name="eventValue">Control state transition</param>
        private void OnStateChanged(HeidenhainDNCLib.DNC_EVT_STATE eventValue)
        {
            ...
            this.BeginInvoke(new OnStateChangedHandler(this.OnStateChangedImpl), eventValue);
        }
#endif
        ...
        private void OnStateChangedImpl(HeidenhainDNCLib.DNC_EVT_STATE machineState)
        {
            try
            {
                this.StateMachineNCK(machineState);
                ...
                switch (machineState)
                {
                    ...
                    case HeidenhainDNCLib.DNC_EVT_STATE.DNC_EVT_STATE_PERMISSION_DENIED:
                        this.AppState = ApplicationState.disconnecting;
                        Debug.WriteLine("--- automatic Disconnect (DNC_EVT_STATE_PERMISSION_DENIED) ---");
                        this.Disconnect();
                        break;
                }

                // --- show state in wpf user control -------------------------------------------------------
                this.wpfOnStateChanged.SetState(this.ControlState);
            }
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            string versionComInterface = Utils.CheckDncVersion(1, 5, 1, 0);

            // Init caption of main window with actual dnc version
            if (!string.IsNullOrEmpty(versionComInterface))
            {
                this.Text += " --> using HeidenhainDNC version: " + versionComInterface;
            }

            try
            {
                // --- create WPF control to show machine state -------------------------------------------
                this.wpfOnStateChanged = new DNC_CSharp_OnStateChanged.OnStateChangedUserControl();

                // use ElementHost to integrate WPF UserControl in WinFroms
                ElementHost elhost = new ElementHost();
                elhost.Child = this.wpfOnStateChanged;
                elhost.Location = new Point(0, 0);
                elhost.Size = new Size(560, 840);
                elhost.Dock = DockStyle.Fill;

                // add controll
                MainTableLayoutPanel.Controls.Add(elhost, 0, 1);

                //// init state
                this.wpfOnStateChanged.SetState(HeidenhainDNCLib.DNC_STATE.DNC_STATE_NOT_INITIALIZED);
            }
            catch (COMException cex)
            {
                string className = this.GetType().ToString();
        ...
        private void ConfigureConnectionButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                object connectionNameObject = ConnectionListComboBox.SelectedIndex.ToString();
...
#endif
                // try to connect
                this.Machine.ConnectRequest(ConnectionListComboBox.Text);
...
#endif
                AutoReconnectCheckBox.Enabled = true;

                // This method also enables the OnStateChanged events.
                // Make sure to advise this event interface first, to prevent missing a state change.
        ...
        private void StateMachineNCK(HeidenhainDNCLib.DNC_EVT_STATE evtState)
        {
            ...
            Debug.WriteLine(this.previousControlState.ToString() + " --> " + this.ControlState.ToString());
        }
        #endregion
